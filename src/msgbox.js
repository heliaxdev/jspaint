...
try {
  ...
  const audio_buffer_promise = fetch(CHORD_WAV_URL)
...
function showMessageBox_implementation({
  ...
  let $window, $message;
  const promise = /** @type {MessageBoxPromise} */ (
    new Promise((resolve) => {
      $window = make_window_supporting_scale(
        Object.assign(
          {
            title,
            resizable: false,
      ...
      $("<div>")
        .append(
          $("<img width='32' height='32'>")
            .attr("src", `images/${iconID}-32x32-8bpp.png`)
            .css({
              margin: "16px",
              display: "block",
            }),
